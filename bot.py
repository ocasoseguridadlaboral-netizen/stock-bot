...
def _read_products(ws):
    ...
    for r_i, row in enumerate(rows, start=2):
        ...
        if not p["Categoria"] and "descripcion" in idx:
            ...
            for w in ["pantalon","bermuda","botin","camisa","remera","campera","chaleco","buzo","zapatilla","guante","jean","gorra","matafuego","recarga"]:
...
def _detect_categoria_from_text(text_nrm: str) -> Optional[str]:
    for w in ["pantalon","bermuda","botin","zapatilla","camisa","remera","campera","chaleco","buzo","guante","jean","gorra","matafuego","recarga"]:
...
def _detectar_intencion(text_nrm: str) -> Optional[str]:
    ...
    if re.search(r"\bfaltan?\b|\bme faltan\b", text_nrm): return "consultar_faltantes_filtrados"
    return None
...
def _cantidad_desde_verbos(text_nrm: str) -> Optional[int]:
    # patrones tipo "x2", "por 3"
    m = re.search(r"\b(?:x|por)\s*(\d{1,4})\b", text_nrm)
    if m: 
        try: return int(m.group(1))
        except: pass
    # palabra número
    for w, n in _NUM_PALABRA.items():
        if re.search(rf"\b{w}\b", text_nrm):
            return n
    return None

def _cantidad_por_sustantivo(text_nrm: str) -> Optional[int]:
    # “2 pantalones …”, “10 camisas …”
    m = re.search(r"\b(\d{1,4})\s+(?:\w{3,})s?\b", text_nrm)
    if m:
        try: return int(m.group(1))
        except: return None
    return None
...
def _separar_cantidad_talle(text_nrm: str, talles_catalogo: set) -> Tuple[int, Optional[str]]:
    """
    Devuelve (cantidad, talle_detectado). 
    Reglas:
      - Si hay '44 x2' → talle=44, cantidad=2
      - Si hay dos números y uno está en talles_catalogo → ese es talle y el otro es cantidad
      - Si solo hay un número y está en talles_catalogo → talle=ese y cantidad (si aparece en palabras/x2), si no, 1
      - Si solo hay un número y NO está en talles_catalogo → cantidad=ese, talle=None
    """
    nums = [int(n) for n in re.findall(r"\b\d{1,4}\b", text_nrm)]
    # x2 junto a talle
    m = re.search(r"\b(\d{2,3})\s*(?:x|por)\s*(\d{1,3})\b", text_nrm)
    if m:
        t, c = int(m.group(1)), int(m.group(2))
        if _normalize(str(t)) in talles_catalogo:
            return max(1,c), str(t)

    # dos números: decidir
    if len(nums) >= 2:
        a, b = nums[0], nums[1]
        a_in = _normalize(str(a)) in talles_catalogo
        b_in = _normalize(str(b)) in talles_catalogo
        if a_in and not b_in:   # “44 2”
            return max(1,b), str(a)
        if b_in and not a_in:   # “2 44”
            return max(1,a), str(b)
        # ambos son talles posibles → dejar talle por contexto y cantidad por verbo/palabra
        cant = _cantidad_desde_verbos(text_nrm) or _cantidad_por_sustantivo(text_nrm) or 1
        # priorizar el que aparezca después de palabra 'talle'
        m2 = re.search(r"talle\s*(\d{2,3})", text_nrm)
        if m2:
            return max(1,cant), m2.group(1)
        # por defecto: último como talle
        return max(1,cant), str(b)

    # un número solo:
    if len(nums) == 1:
        n = nums[0]
        if _normalize(str(n)) in talles_catalogo:
            cant = _cantidad_desde_verbos(text_nrm) or _cantidad_por_sustantivo(text_nrm) or 1
            return max(1,cant), str(n)
        else:
            return max(1,n), None

    # sin números claros
    cant = _cantidad_desde_verbos(text_nrm) or _cantidad_por_sustantivo(text_nrm) or 1
    return max(1,cant), None
...
def _nlp_parse(text: str) -> Dict[str, Any]:
    ...
    if accion == "consultar_faltantes_filtrados":
        return {"intent":"faltantes_filtrados", "data":{"query":text}}

    # default ajustar stock
    cantidad = 1  # ya lo ajustaremos con _separar_cantidad_talle
    ...
    if OPENAI_API_KEY and OpenAI:
        try:
            ...
            system = (
                ...
                "Devolvé JSON con 'intent' y 'data'. intents: 'ajustar_stock','reporte','faltantes','ganancias','anular_ultimo','faltantes_filtrados'. "
                "Si el texto implica anular/deshacer, devolvé intent='anular_ultimo'. "
                "Si el texto pregunta 'qué ... me faltan', devolvé intent='faltantes_filtrados' y pasá en data.query el texto original. "
...
def _build_item_from_text(products, text: str, accion_global: Optional[str]) -> Dict[str, Any]:
    ...
    # acción
    accion = data.get("accion") or accion_global or ("agregar" if re.search(r"\bcompr|agreg|sum", _normalize(text)) else "descontar")
    # entidades
    ...
    # cantidad y talle desambiguados
    talles_catalogo = { _normalize(p.get("Talle","")) for p in products if p.get("Talle") }
    cant_auto, talle_auto = _separar_cantidad_talle(_normalize(text), talles_catalogo)
    if not filtro.get("Talle") and talle_auto:
        filtro["Talle"] = talle_auto
    cantidad = int((data or {}).get("cantidad") or cant_auto or 1)
    precio_venta = (data or {}).get("precio_venta")
...
def _descripcion_corta(p: Dict[str,Any]) -> str:
    ...
    base = f"{cat} {p.get('Producto','')}".strip()
    det = " ".join([x for x in [p.get('Talle',''), p.get('Color','')] if x]).strip()
    return (base + (" " + det if det else "")).strip()
...
async def _preguntar_slot(update: Update, slot: str, products: List[Dict[str,Any]]):
    if slot == "Categoria":
        await update.message.reply_text("¿Es *pantalon*, *bermuda*, *botin*, *zapatilla*, *matafuego*, etc.?", parse_mode="Markdown")
    elif slot == "Producto":
        await update.message.reply_text("¿Qué *producto* es? (ej. 'clasico', 'cargo', 'ripstop', 'frances')", parse_mode="Markdown")
    elif slot == "Talle":
...
async def _cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "Ejemplos:\n"
        "• 'Compré 4 pantalones cargo 50 azul'\n"
        ...
        "• '¿qué pantalones ripstop me faltan?' | 'qué pantalones clásicos verde me faltan'\n"
        "• 'anular' o /anular → deshace el último movimiento\n\n"
        "Comandos: /reporte, /faltantes, /faltan, /ganancias, /agregar, /anular, /ping, /version, /estado"
    )
...
async def _cmd_faltan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """ /faltan categoria=... producto=... color=... talle=... """
    _, ws, _ = _open_sheet()
    products, _ = _read_products(ws)

    filtros = {"Categoria":"", "Producto":"", "Color":"", "Talle":""}
    text = " ".join(context.args or [])
    # parse estilo clave=valor
    for m in re.finditer(r"(categoria|producto|color|talle)\s*=\s*([^\s,]+)", text.lower()):
        k, v = m.group(1), m.group(2)
        if k == "categoria": filtros["Categoria"] = v
        elif k == "producto": filtros["Producto"] = v
        elif k == "color": filtros["Color"] = v
        elif k == "talle": filtros["Talle"] = v

    # Si no vinieron args, intentar leer desde el mensaje completo (usuario suele escribir texto)
    if not any(filtros.values()):
        filtros = _extract_entities_from_query(products, text)

    # filtrar faltantes
    candidatos = _filtrar_por_campos(products, filtros)
    falt = [p for p in candidatos if p["Stock"] <= p["Minimo"]]
    if not falt:
        await update.message.reply_text("✅ No hay faltantes para ese filtro.")
        return
    cab = "🚨 *Faltantes filtrados:*"
    det = "\n".join([f"- {_descripcion_corta(p)} ({p['Stock']} / mín {p['Minimo']})" for p in falt[:300]])
    await update.message.reply_markdown((cab + "\n" + det)[:4000])
...
async def _on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(w in text_nrm for w in UNDO_WORDS):
        ...
        return
    if re.search(r"\bfaltan?\b|\bme faltan\b", text_nrm):
        # consulta de faltantes con filtros en lenguaje natural
        _, ws, _ = _open_sheet()
        products, _ = _read_products(ws)
        filtros = _extract_entities_from_query(products, text)
        candidatos = _filtrar_por_campos(products, filtros)
    ...
    if len(items) <= 1:
        ...
        if intent in ("reporte","faltantes","ganancias"):
            ...
            return
        if intent == "faltantes_filtrados":
            filtros = _extract_entities_from_query(products, data.get("query") or text)
            candidatos = _filtrar_por_campos(products, filtros)
            falt = [p for p in candidatos if p["Stock"] <= p["Minimo"]]
            if not falt:
                await update.message.reply_text("✅ No hay faltantes para ese filtro.")
        ...
        # desambiguar cantidad/talle
        talles_catalogo = { _normalize(p.get("Talle","")) for p in products if p.get("Talle") }
        cant_auto, talle_auto = _separar_cantidad_talle(text_nrm, talles_catalogo)
        cantidad = int(data.get("cantidad") or cant_auto or 1)
        precio_venta = data.get("precio_venta")

        filtro = _extract_entities_from_query(products, text)
        for k_src, k_dst in [("categoria","Categoria"), ("producto","Producto"), ("talle","Talle"), ("color","Color"), ("codigo","Codigo")]:
            v = data.get(k_src) if data else None
            if v and not filtro.get(k_dst):
                filtro[k_dst] = _normalize(v)
...
def main():
    ...
    app.add_handler(CommandHandler("faltan", _cmd_faltan))
